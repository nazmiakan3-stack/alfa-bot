...
def analyze(symbol):
  ...
  k_last2, d_last2 = calc_stoch_rsi(
      closed_closes, period=14, stoch_period=14, k_period=3, d_period=3
  )
...
def main():
  ...
  send_telegram_msg(
      f"👑 <b>MEXC ALFA BOT BAŞLATILDI!</b>\nTarih: {now_date_text()}\nBorsa:"
  ...
  while True:
    try:
      ...
      for symbol, name in SYMBOLS.items():
        signal, current_price, stoch_k, cur_high, cur_low = analysis_dict.get(
        ...
        if (
          ...
          trade_events.append(
              "🚨 <b>MEXC KESKİN NİŞANCI GİRİŞİ!</b>\n"
        ...
        if pos is not None:
          ...
          if "highest_price" not in pos:
            pos["highest_price"] = entry  # Eski veriler hata vermesin diye
          ...
          if hit_sl:
            ...
            final_pnl = (POSITION_SIZE * exit_pct) - (
      ...
      pnl_pct = (
          (total_unrealized_pnl / total_equity * 100) if total_equity > 0 else 0.0
      )
